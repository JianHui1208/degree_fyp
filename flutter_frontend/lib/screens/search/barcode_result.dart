...
class BarcodeResult extends StatefulWidget {
  ...
  const BarcodeResult({super.key, required this.barcode});
...
class _BarcodeResultState extends State<BarcodeResult> {
  ...
  void searchItemUseBarcode() async {
    String itemString = await searchItemByBarcode(widget.barcode);
    print(itemString);
    itemObj = jsonDecode(itemString);

    if (itemObj?['status'] == 1110) {
      ...
      const ErrorBox(
        message: 'Error',
        content: 'Item not found, wait for server response',
      );
    }
  ...
  void deleteItemFunc() async {
    String itemString = await deleteItem(widget.barcode.toString());
  ...
  void initState() {
    super.initState();
    searchItemUseBarcode();
  }

  @override
  Widget build(BuildContext context) {
    return ScaffoldCustom(
      text: 'Search Barcode Detail',
